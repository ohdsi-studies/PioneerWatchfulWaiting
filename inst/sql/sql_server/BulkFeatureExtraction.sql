...
{DEFAULT @procedure_group_analysis_id = 712}

@cohort_subset_table_create

@feature_time_window_table_create

IF OBJECT_ID ('tempdb..#cov_ref', 'U') IS NOT NULL DROP TABLE #cov_ref;
...
FROM @cohort_database_schema.@cohort_table C
    INNER JOIN #cohort_subset xref
ON xref.cohort_id = C.cohort_definition_id
...
SELECT cohort.cohort_definition_id,
       CAST(FLOOR((YEAR(cohort_start_date) - year_of_birth) / 5) * 1000 +
            @age_group_analysis_id AS BIGINT) AS covariate_id,
...
INSERT INTO #cov_ref (covariate_id,
                      covariate_name,
                      analysis_id,
                      concept_id)
SELECT covariate_id * 10 + 1, -- To match way the cohort construction is done when running sequentially in R
       CAST(CONCAT(
               'age group: ',
...
/*  Gender   */
/*************/
IF OBJECT_ID('tempdb..#gender', 'U') IS NOT NULL
DROP TABLE #gender
;
...
INSERT INTO #cov_ref (covariate_id,
                      ...
                      concept_id)
SELECT covariate_id * 10 + 1, -- To match way the cohort construction is done when running sequentially in R
       CAST(CONCAT('gender = ', CASE
                                    WHEN concept_name IS NULL
...
-- Consolidate results
INSERT INTO #analysis_results (cohort_id,
                               covariate_id,
                               sum_value,
                               mean,
                               sd)
...
ON f.cohort_definition_id = C.cohort_definition_id
;
...
IF OBJECT_ID('tempdb..#dg_covariate_table', 'U') IS NOT NULL
...
INSERT INTO #cov_ref (covariate_id,
                      ...
                      analysis_id,
                      concept_id)
SELECT covariate_id * 10 + window_id,
       CAST(CONCAT('drug_era group during day ', t1.window_start,
                   CASE
                       WHEN t1.window_end > 100000
                           THEN '+: '
                       ELSE CONCAT(' through ', t1.window_end, ' days relative to index: ')
                   END,
                   CASE
                       WHEN concept_name IS NULL
                           THEN 'Unknown concept'
...
FROM (
     ...
     FROM #dg_covariate_table cov
         INNER JOIN #feature_windows W
     ON cov.window_id = W.window_id
     ) t1
LEFT JOIN @cdm_database_schema.concept
...
FROM @cdm_database_schema.concept_ancestor
    ...
    SELECT *
    FROM @cdm_database_schema.concept_ancestor
    WHERE ancestor_concept_id IN (4042485, 4097527, 4217238, 4300757, 4305790)
...
IF OBJECT_ID('tempdb..#pg_covariate_table', 'U') IS NOT NULL
DROP TABLE #pg_covariate_table;

SELECT cohort_definition_id,
       window_id,
       CAST(ancestor_concept_id AS BIGINT) * 1000 + @procedure_group_analysis_id AS covariate_id,
       sum_value
INTO #pg_covariate_table
FROM (
    SELECT cohort.cohort_definition_Id, cohort.window_id, g1.ancestor_concept_id,
    ...
    GROUP BY cohort.cohort_definition_Id, cohort.window_id, g1.ancestor_concept_id
    ) TEMP
;


-- Reference construction
INSERT INTO #cov_ref (covariate_id, covariate_name, analysis_id, concept_id)
SELECT covariate_id * 10 + window_id,
       CAST(CONCAT('procedure group during day ', t1.window_start,
                   CASE
                       WHEN t1.window_end > 100000
                           THEN '+: '
                       ELSE CONCAT(' through ', t1.window_end, ' days relative to index: ')
                   ...
                   CASE
                       ...
                       ELSE concept_name
                   END) AS VARCHAR(512)) AS covariate_name,
       @procedure_group_analysis_id AS analysis_id,
       CAST((covariate_id - @procedure_group_analysis_id) / 1000 AS INT) AS concept_id
FROM (
     SELECT DISTINCT cov.covariate_id, w.window_id, w.window_start, w.window_end
     FROM #pg_covariate_table cov
         INNER JOIN #feature_windows W
     ON cov.window_id = W.window_id
     ) t1
LEFT JOIN @cdm_database_schema.concept
    ON concept_id = CAST((covariate_id - @procedure_group_analysis_id) / 1000 AS INT);
...
IF OBJECT_ID('tempdb..#cg_covariate_table', 'U') IS NOT NULL
...
INSERT INTO #cov_ref (covariate_id,
                      ...
                      concept_id)
SELECT covariate_id * 10 + window_id,
       CAST(CONCAT('condition_era group during day ', t1.window_start,
                   CASE
                       WHEN t1.window_end > 100000
                           THEN '+: '
                       ELSE CONCAT(' through ', t1.window_end, ' days relative to index: ')
...
TRUNCATE TABLE #pg_covariate_table;
DROP TABLE #pg_covariate_table;
